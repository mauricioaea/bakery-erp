...
def antes_de_cada_peticion():
    # =============================================
    # 🆕 SAAS - DETECCIÓN Y CONFIGURACIÓN DE TENANT
    # =============================================
    from middleware_saas import gestor_tenants
    
    # 1. Detectar y configurar tenant
    gestor_tenants.detectar_y_configurar_tenant()
    
    # 2. Configurar SQLAlchemy para el tenant detectado
    if hasattr(g, 'tenant'):
        app.config['SQLALCHEMY_DATABASE_URI'] = f"sqlite:///databases_tenants/{g.tenant['base_datos']}"
        print(f"🔧 SaaS - BD configurada: {g.tenant['base_datos']}")
    else:
        app.config['SQLALCHEMY_DATABASE_URI'] = "sqlite:///databases_tenants/panaderia_principal.db"
        print("🔧 SaaS - BD por defecto: panaderia_principal.db")
