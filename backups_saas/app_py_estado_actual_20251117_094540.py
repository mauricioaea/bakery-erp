...
import uuid
import json
\1, gnder_template, request, redirect, url_for, session, flash, jsonify, Response, make_response
...
+ os.path.join(basedir, 'panaderia.db')
...
# =============================================
# 🆕 MIDDLEWARE UNIFICADO SAAS - SOLO UN before_request
# =============================================

@app.before_request
def middleware_unificado_saas():
    """Middleware global unificado - VERSIÓN SAAS MEJORADA"""
    
    # 1. CONFIGURACIÓN SAAS - Detectar tenant y configurar BD
    if hasattr(g, 'tenant'):
        # Usar BD del tenant detectado
        app.config['SQLALCHEMY_DATABASE_URI'] = f"sqlite:///databases_tenants/{g.tenant['base_datos']}"
    else:
        # Fallback a BD principal (para inicialización)
        app.config['SQLALCHEMY_DATABASE_URI'] = "sqlite:///databases_tenants/panaderia_principal.db"
    
    # 2. INFORMACIÓN USUARIO Y PANADERÍA
    from multicliente_middleware import obtener_info_usuario
    obtener_info_usuario()

    # 3. VERIFICACIÓN SUSCRIPCIÓN (código existente)
